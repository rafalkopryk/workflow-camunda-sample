...
public record ApplicationRegistered(string ApplicationId, decimal Amount, int CreditPeriodInMonths, decimal AverageNetMonthlyIncome, string Pesel, string ProcessCode) : IEvent;
    ...
    ) : IRequestHandler<RegisterApplicationCommand, RegisterApplicationCommandResponse>
{
    ...
    public async Task<RegisterApplicationCommandResponse> Handle(RegisterApplicationCommand command, CancellationToken cancellationToken)
    {
        ...
        await _bus.PublishAsync(new ApplicationRegistered(
                ...
                command.ProcessCode), deliveryOptions);
